...
#include "my_spi.h" // 确保包含了my_spi的硬件驱动

// RAM中的双缓冲区
int32 distance_memory[MAX_POINTS];
float yaw_memory[MAX_POINTS];
...
void save_total_points_to_flash(uint16 total_points) {
	...
	// 擦除存储总点数的扇区 (通常是扇区0)
	easrse_spi(addr_h, addr_m, addr_l); // 内部会等待擦除完成
	P63 = 0; // 保持原有的P63操作，如果它有特定意义
...
void navigation_memory_test(void) {
	...
	flash_read_pointer = 0; // 初始时，Flash读取指针指向路径的起始
	// 首次填充RAM缓冲区 (MAX_POINTS个点)
	for (i = 0; i < 31 && flash_read_pointer < total_points_saved; i++) {
		read_navigation_point_from_flash(&distance_memory[i], &yaw_memory[i], flash_read_pointer);
		flash_read_pointer++;
	}
	for (i = 0; i < 31; i++) {
