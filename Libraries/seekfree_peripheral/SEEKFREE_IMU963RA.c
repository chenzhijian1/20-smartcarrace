...
{
    uint16 edata j=IMU963RA_SOFT_IIC_DELAY;
    while(j--);
}
...
{
    uint8 edata return_state = 0;
    uint16 edata timeout_count = 0;
    addr = addr << 1;
...
// 参数说明     void
// 返回参数     uint8 edata           1-自检失败 0-自检成功
...
// 函数简介     初始化 IMU963RA
// 参数说明     void
// 返回参数     uint8 edata           1-初始化失败 0-初始化成功
// 使用示例     imu963ra_init();
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
